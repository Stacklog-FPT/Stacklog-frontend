...
export default function ModalAI({ placement = "bottom-right" }) {
  ...
  const [showListPriorityDropdown, setShowListPriorityDropdown] =
    useState(false);
  ...
  useEffect(() => {
    const onDocClick = (e) => {
      ...
      if (
        listPriorityRef.current &&
        !listPriorityRef.current.contains(e.target)
      ) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setHistory((h) => [
        ...h,
        { from: "ai", text: `Auto-reply to: "${prompt}"` },
      ]);
      setQuery("");
    } finally {
      setLoading(false);
  ...
  const handleGenerate = async () => {
    if (mode === "single") {
      ...
      try {
        ...
        const data = await postAiTaskAndDispatch({
          title: apiTitle,
          payload: body,
          token: user?.token,
          dispatch,
  ...
  const handleCopy = async () => {
    try {
      ...
      setHistory((h) => [
        ...h,
        { from: "ai", text: "JSON copied to clipboard" },
      ]);
    } catch (e) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setHistory((h) => [
        ...h,
        { from: "ai", text: "Confirmed: JSON copied to clipboard" },
      ]);
              ...
              <div className="modal-ai-box">
                ...
                <div className="modal-ai-body">
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            <button
                              ...
                              className="modal-ai-priority-btn"
                              onClick={() =>
                                setShowListPriorityDropdown((s) => !s)
                              }
                            >
                              <span
                                className={`modal-ai-priority-dot modal-ai-priority-${listPriority.toLowerCase()}`}
                              />
                              <span style={{ marginLeft: 8 }}>
                                {listPriority || "Select"}
                              </span>
                        ...
                        <div className="modal-ai-task-list">
                          ...
                          ((generatedList && generatedList.length > 0) ||
                            ...
                            (generatedList && generatedList.length > 0
                              ...
                              <ModalAITaskItem
                                key={
                                  t.taskId || t.TaskId || t.id || Math.random()
                                }
                                task={t}
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }} />
                    <div style={{ color: "#6b7280" }}>
                      {generatedJson ? "Preview task" : "No tasks previewed"}
                    </div>
                  </div>
                  ...
                  <div className="modal-ai-history">
                    {generatedList && generatedList.length > 0 ? (
                      ...
                      history.map((m, i) => (
                        <div
                          key={i}
                          className={`modal-ai-msg modal-ai-msg-${m.from}`}
                        >
