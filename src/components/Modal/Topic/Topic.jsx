...
const PlanComponent = () => {
  ...
  const globalSelectedClassId = useSelector((state) => state.semester?.currentClassId || null);
  ...
  useEffect(() => {
    ...
    if (globalSelectedClassId && !classes.some((cls) => cls.classesId === globalSelectedClassId)) {
      setCurrentGroupId('');
    }
  ...
  const filteredTopics = useMemo(() => {
    ...
    let list =
      role === 'LECTURER'
        ? safePlans.filter((t) => groupMap[t.groupId]?.classId === effectiveClassId)
        : currentGroupId
        ? safePlans.filter(
            (t) =>
              t.groupId === currentGroupId && groupMap[t.groupId]?.classId === effectiveClassId,
          )
        : [];
  ...
  }, [role, normalizedPlans, groupMap, effectiveClassId, currentGroupId, statusFilter, keyword]);
        ...
        <table className="sl-table">
          ...
          <tbody>
            {pending ? (
              ...
              <tr>
                <td colSpan={7}>
                  <div className="sl-empty">No data matches the current filters.</div>
                </td>
              </tr>
            ) : (
