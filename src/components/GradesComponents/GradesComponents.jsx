...
import { saveScoreCategory, getScoreCategoriesByClass } from "../../service/ScoreService";
...
const GradesComponents = ({ handleActiveDetail, handleActivityAddCore }) => {
  ...
  const uid = String(user?.userId ?? user?.user_id ?? user?.work_id ?? user?.workId ?? user?._id ?? user?.id ?? user?.sub ?? '');
  const tokenPayload = decodeToken(token);
  const tokenId = String(tokenPayload?.id ?? tokenPayload?._id ?? tokenPayload?.userId ?? tokenPayload?.sub ?? '');
  ...
  const [newCategory, setNewCategory] = React.useState({ scoreCategoryName: '', scoreCategoryWeight: 0, scoreCategoryComment: '' });
    ...
    try {
      ...
      let mapped = results
        ...
        .map((user) => ({
          ...
          average: typeof user.personal_score === 'number' ? user.personal_score : 0,
          avatar: user.avatar_link ? (user.avatar_link.startsWith('http') ? user.avatar_link : `${user.avatar_link}`) : '',
        }));
      ...
      try {
        if (selectedClassId && token) {
          let cats = [];
          try {
            const data = await getScoreCategoriesByClass(selectedClassId, token, dispatch);
            cats = Array.isArray(data) ? data : (Array.isArray(data?.data) ? data.data : []);
          } catch (e) {
            console.warn('Could not fetch categories for averaging, falling back to personal_score', e?.message || e);
            cats = [];
          }
          ...
          if (cats && cats.length > 0) {
            ...
            mapped = mapped.map((u) => {
              ...
              const extractGSId = (s) => String(s?.userId ?? s?.user_id ?? s?.groupStudentId ?? s?.studentId ?? s?.id ?? (s?.user?._id) ?? '').trim();
              const extractGSEmail = (s) => String(s?.email ?? s?.user?.email ?? '').toLowerCase();
              const extractGSWorkId = (s) => String(s?.work_id ?? s?.user?.work_id ?? s?.workId ?? '').toLowerCase();

              const rawMatch = (groupStudents || []).find((gs) => {
                const sid = extractGSId(gs);
                if (!sid) return false;
                // compare with mapped user ids/emails
                const candidates = new Set([u._id, u.id, u.user_id, String(u.id || ''), String(u._id || '')].filter(Boolean).map(String));
                if (candidates.has(sid)) return true;
                const semail = extractGSEmail(gs);
                if (semail && u.email && String(u.email).toLowerCase() === semail) return true;
                const swork = extractGSWorkId(gs);
                if (swork && u.id && String(u.id).toLowerCase() === swork) return true;
                return false;
              });
              ...
              cats.forEach((c) => {
                ...
                const si = c.scoreItems.find((si) => {
                  const sid = String(si.userId ?? si.user_id ?? si._id ?? si.id ?? '').trim();
                  if (sid && uidCandidates.has(sid)) return true;
                  // fallback: match by email if available
                  const siEmail = String(si.email ?? si.userEmail ?? si.user?.email ?? '').toLowerCase();
                  if (siEmail && u.email && String(u.email).toLowerCase() === siEmail) return true;
                  return false;
                });
                const val = si && si.scoreItemValue !== null && si.scoreItemValue !== undefined && !Number.isNaN(Number(si.scoreItemValue)) ? Number(si.scoreItemValue) : NaN;
                if (!Number.isNaN(val)) {
                  weightedSum += val * w;
                  sumWeights += w;
                }
              });
              const avg = sumWeights > 0 ? (weightedSum / sumWeights) : (typeof u.average === 'number' ? u.average : 0);
              return { ...u, average: Number(avg.toFixed(2)), status: avg >= 5 };
            });
          }
        }
      } catch (e) {
      ...
      if (isStudent) {
        ...
        const candidateIds = new Set([uid, tokenId, String(user?.user_id || ''), String(user?.work_id || ''), String(user?._id || ''), String(user?.id || '')].filter(Boolean));
        const candidateEmail = user?.email ? String(user.email).toLowerCase() : '';
        const candidateUsername = user?.username ? String(user.username).toLowerCase() : '';
        ...
        const matches = mapped.filter((m) => {
          ...
          if (candidateEmail && memail && candidateEmail === memail) return true;
          if (candidateUsername && mname && mname.includes(candidateUsername)) return true;
          return false;
        });
        ...
        if (matches.length > 0) {
          ...
          const rawMatch = groupStudents.find((gs) => {
            ...
            const semail = String(gs?.email ?? gs?.user?.email ?? '').toLowerCase();
            if (candidateEmail && semail && candidateEmail === semail) return true;
            const susername = String(gs?.username ?? gs?.user?.username ?? gs?.user_name ?? '').toLowerCase();
            if (candidateUsername && susername && susername.includes(candidateUsername)) return true;
            return false;
          });
          if (rawMatch) {
            mapped = [{
              _id: rawMatch.groupStudentId || rawMatch.userId || String(rawMatch.id || ''),
              name: rawMatch.name || rawMatch.userId || candidateUsername || 'You',
  ...
  console.debug('[GradesComponents] tokenPayload, tokenId:', tokenPayload, tokenId);
  console.debug('[GradesComponents] getClasses result:', data);
        ...
        const groups = normalized.flatMap((c) => (c.groups || []).map((g) => ({
  ...
  setArea(uniq);
        ...
        if (normalized.length > 0) {
          ...
          if (firstGroups.length > 0) {
            console.debug('[GradesComponents] firstGroups for firstClass:', firstGroups.map(f => ({ id: f._id, students: (f.raw?.groupStudents || []).length }))); 
            ...
            if (isStudent) {
              ...
              if (!uid) {
                ...
                console.debug('[GradesComponents] decoded token payload (no uid):', tokenPayload, tokenId);
              }
              const extractGSId = (s) => String(s?.userId ?? s?.user_id ?? s?.groupStudentId ?? s?.studentId ?? s?.id ?? (s?.user?._id) ?? '');
              const extractGSEmail = (s) => String(s?.email ?? s?.user?.email ?? s?.user_email ?? '').toLowerCase();
              const extractGSUsername = (s) => String(s?.username ?? s?.user?.username ?? s?.user_name ?? '').toLowerCase();
              const extractGSWorkId = (s) => String(s?.work_id ?? s?.user?.work_id ?? s?.workId ?? s?.user?.workId ?? '').toLowerCase();
              const userAltIds = new Set([uid, tokenId, String(user?.user_id || ''), String(user?.work_id || ''), String(user?._id || ''), String(user?.id || '')].filter(Boolean));
              const userEmail = user?.email ? String(user.email).toLowerCase() : '';
              const userName = user?.username ? String(user.username).toLowerCase() : '';
              const userWorkId = user?.work_id ? String(user.work_id).toLowerCase() : '';
              const myGroup = firstGroups.find((g) => Array.isArray(g.raw?.groupStudents) && g.raw.groupStudents.some((s) => {
                ...
                const matchedByUsername = userName && susername && userName === susername;
                const matchedByWorkId = userWorkId && swork && userWorkId === swork;
                const matched = matchedById || matchedByEmail || matchedByUsername || matchedByWorkId;
                if (!matched) console.debug('[GradesComponents] group student id mismatch', { sid, semail, susername, swork, uid, alt: Array.from(userAltIds), userEmail, userName, userWorkId });
                return matched;
              }));
              if (myGroup) {
                setArea([myGroup]);
                ...
                console.debug('[GradesComponents] matched groupStudents ids:', groupStudents.map(gs => extractGSId(gs)));
  ...
  const handleClassChange = (e) => {
    ...
    const cls = classes.find((c) => String(c.classesId) === String(classId) || String(c._id) === String(classId));
  ...
  console.debug('[GradesComponents] tokenPayload, tokenId:', tokenPayload, tokenId);
  console.debug('[GradesComponents] groups for selected class:', groups.map(g => ({ id: g._id, count: (g.raw?.groupStudents || []).length })) );
    if (isStudent) {
      if (!uid) {
        console.debug('[GradesComponents] user object (no uid):', user);
        console.debug('[GradesComponents] decoded token payload (no uid):', tokenPayload, tokenId);
  ...
  const userAltIds = new Set([uid, tokenId, String(user?.user_id || ''), String(user?.work_id || ''), String(user?._id || ''), String(user?.id || '')].filter(Boolean));
      ...
      const userName = user?.username ? String(user.username).toLowerCase() : '';
      const userWorkId = user?.work_id ? String(user.work_id).toLowerCase() : '';
      const myGroup = groups.find((g) => Array.isArray(g.raw?.groupStudents) && g.raw.groupStudents.some((s) => {
        const sid = extractGSId(s);
        const semail = extractGSEmail(s);
        const susername = extractGSUsername(s);
        const swork = extractGSWorkId(s);
        ...
        const matchedByWorkId = userWorkId && swork && userWorkId === swork;
        const matched = matchedById || matchedByEmail || matchedByUsername || matchedByWorkId;
        if (!matched) console.debug('[GradesComponents] handleClassChange group student id mismatch', { sid, semail, susername, swork, uid, alt: Array.from(userAltIds), userEmail, userName, userWorkId });
        return matched;
      }));
      if (myGroup) {
        setArea([myGroup]);
      ...
      <div className="grades__component__container">
        <div className="grades__component__container__filter__class">
          <div className="grades__component__container__filter__class__feature">
            <select aria-label="Select class" onChange={handleClassChange} value={selectedClassId}>
              {loading ? (
                ...
                <>
                  ...
                  {classes.map((item) => (
                    <option key={item.classesId || item._id} value={item.classesId || item._id}>
            ...
            <select aria-label="Select group" onChange={handleGroupChange} value={selectedGroupId} disabled={isStudent}>
              {loading ? (
                <option value="" disabled>
                  Loading...
                </option>
          ...
          <div className="modal__overlay">
            <div className="modal__card">
              ...
              <div className="modal__fields">
                <div className="modal__field">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewCategory({ ...newCategory, scoreCategoryName: e.target.value })}
                ...
                <div className="modal__field">
                  ...
                  <div className="input--percent">
                    <input
                      ...
                      min={0}
                      max={100}
                      value={typeof newCategory.scoreCategoryWeight === 'number' ? (newCategory.scoreCategoryWeight * 100) : ''}
                      onChange={(e) => {
                        const val = parseFloat(e.target.value || 0);
                        setNewCategory({ ...newCategory, scoreCategoryWeight: isNaN(val) ? 0 : val / 100 });
                      }}
                      aria-label="Category weight percent"
                    />
                    <span className="percent__suffix">%</span>
                  </div>
                  <small className="modal__hint">Enter percentage, e.g. 10 for 10%</small>
                </div>
                ...
                <div className="modal__field">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewCategory({ ...newCategory, scoreCategoryComment: e.target.value })}
                    rows={3}
              ...
              <div className="modal__actions">
                <button onClick={() => setIsAddOpen(false)} disabled={savingCategory}>Cancel</button>
                <button
                  onClick={async () => {
                    // only lecturers allowed, defensive check
                    if (user?.role !== 'LECTURER') return;
                    // ensure class selected
                    const classId = selectedClassId || (classes[0] && (classes[0].classesId || classes[0]._id));
                    if (!classId) {
                      alert('Please select a class first');
                      return;
                    }
                    const comment = (newCategory.scoreCategoryComment || '').trim();
                    const payload = {
                    ...
                    try {
                      ...
                      let existing = [];
                      try {
                        const data = await getScoreCategoriesByClass(classId, token, dispatch);
                        existing = Array.isArray(data) ? data : (Array.isArray(data?.data) ? data.data : []);
                      } catch (e) {
                        // if fetch fails, allow create to proceed (server will still validate) but warn
                        console.warn('Could not fetch existing categories to validate total weight, proceeding with server-side validation', e?.message || e);
                        existing = [];
                      }

                      const sumExisting = existing.reduce((s, c) => s + (typeof c.scoreCategoryWeight === 'number' ? c.scoreCategoryWeight : 0), 0);
                      const proposedTotal = sumExisting + (payload.scoreCategoryWeight || 0);
                      const EPS = 1e-9;
                      if (proposedTotal - 1.0 > EPS) {
                        alert('Cannot create category — total weight would exceed 100%');
                      ...
                      try { await getScoreCategoriesByClass(classId, token, dispatch); } catch(e) { /* ignore */ }
                      setIsAddOpen(false);
                      setNewCategory({ scoreCategoryName: '', scoreCategoryWeight: 0, scoreCategoryComment: '' });
                    } catch (e) {
          ...
          <table>
            ...
            <tbody>
              {studentsLoading ? (
                ...
                currentItems.map((item, index) => (
                    ...
                    <td>
                      <div className="name__ava">
                        <img src={item.avatar || '/public/default-avatar.png'} alt={item.name || 'avatar'} />
                        <p>{item.name}</p>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      {(() => {
                        const raw = typeof item.average === 'number' ? item.average : (parseFloat(item.average) || 0);
                    ...
                    <td>
                      <span
                        ...
                        onClick={() => handleActiveDetail({ student: item, classId: selectedClassId, groupId: selectedGroupId })}
                      >
                        Detail
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={7} style={{ textAlign: 'center', padding: '16px' }}>
                    {selectedClassId && selectedGroupId ? 'No students in this group.' : 'Please select a class and group.'}
                  </td>
                </tr>
              )}
            </tbody>
