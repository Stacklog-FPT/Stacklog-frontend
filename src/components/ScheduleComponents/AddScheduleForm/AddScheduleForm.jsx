    ...
    const load = async () => {
      ...
      try {
        ...
        const list = results
          ...
          .filter((x) => x.user)
          .map((x) => ({ id: x.id, full_name: x.user?.full_name || x.user?.name || x.id, avatar: x.user?.avatar }));
        ...
        <form className="add-schedule-form" onSubmit={handleSubmit}>
          ...
          {studentsList.length > 0 && (
            <div className={`form-group students-checklist ${showStudentsPanel ? 'open' : ''}`}>
              ...
              {showStudentsPanel && (
                <div ref={studentsPanelRef} className="students-panel" role="dialog" aria-label="Select members">
                  {studentsList.map((s) => (
